...
export default function Contact() {
  ...
  const socialLinks = [
    {
      href: 'https://www.github.com/usernumber89',
    ...
    {
      href: 'https://x.com/roantocampos08',
      icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {
      href: 'https://www.instagram.com/rocampos04',
  ...
  return (
    <section id="contact" className="pt-20 lg:pt-32 pb-16">
      <div className="px-4 mx-auto lg:px-8 xl:max-w-5xl">
        ...
        <div className="flex justify-center gap-3 mb-12">
          {socialLinks.map(({ href, icon, alt, label }, key) => (
            <a 
              key={key} 
              href={href} 
              target="_blank" 
              rel="noopener noreferrer" 
              className="group relative p-3 text-zinc-400 hover:text-zinc-100 bg-zinc-800/50 hover:bg-zinc-800 rounded-xl transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-[#2196F3]/20"
              aria-label={label}
            >
              <span className="relative z-10">{icon}</span>
              <span className="absolute inset-0 rounded-xl bg-gradient-to-r from-[#2196F3]/0 to-[#64B5F6]/0 group-hover:from-[#2196F3]/10 group-hover:to-[#64B5F6]/10 transition-all duration-300" />
            </a>
          ))}
        </div>

       
        <form method='POST' action="https://getform.io/f/bnlnpmwb" className="max-w-2xl mx-auto space-y-5">
